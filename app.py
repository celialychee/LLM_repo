...
class StreamlitUI:
    ...
    def setup_sidebar(self):
        ...
        model_name_value = os.environ.get('LLM_MODEL_NAME', None)
        if model_name_value is None:
            raise ValueError(
                'Please set LLM_MODEL_NAME either in the environment as LLM_MODEL_NAME')
        model_name = st.sidebar.text_input('模型名称：', value=model_name_value)
        ...
        model_ip = st.sidebar.text_input('模型IP', value=f'{API_CONFIG.LLM_ROUTER_NAME}:23333')
        ...
        if WEB_CONFIGS.ENABLE_ASR:
            ...
            if len(audio) > 0:
                ...
                return model_name, model, plugin_action, uploaded_file, model_ip, asr_text
...
def main():
    ...
    vars_list = st.session_state[
        'ui'].setup_sidebar()
    if len(vars_list)==5:
        _, model, plugin_action, uploaded_file, _ = vars_list
        asr_text = ""
    elif len(vars_list)==6:
        _, model, plugin_action, uploaded_file, _, asr_text = vars_list
    ...
    # 添加语音输入判断
    if user_input := st.chat_input('') or (asr_text != "" and st.session_state["asr_text_cache"] != asr_text):
        if asr_text != "" and st.session_state["asr_text_cache"] != asr_text:
            user_input = asr_text
        with st.container():
